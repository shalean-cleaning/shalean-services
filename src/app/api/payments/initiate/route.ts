...
// Helper function to get customer email from session (removed - using direct session access)
...
async function upsertPaymentRecord(
  ...
  if (existingPayment) {
    // Update existing payment if it's not already paid
    ...
    const { error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        reference,
        amount: amountMinor / 100, // Convert back to major units for display
        amount_minor: amountMinor,
        status,
        gateway: 'paystack'
      });
...
export async function POST(req: Request) {
  try {
    ...
    if (!bookingValidation.isValid) {
      const errorCode = bookingValidation.errorCode || 'validation_failed';
      let statusCode = 400;
